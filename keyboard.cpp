...
//0x60是键盘读写缓冲区
//0x64是键盘控制寄存器端口
//从控制寄存器判断--为0 表示Output buffer为空 / 为1 表示 Input buffer为空
KeyBoardDriver::KeyBoardDriver(InterruptManager* manager)
    :InterruptHandler(0x01 + manager->HardwareInterruptOffset(), manager),
    dataport(0x60),
    commandport(0x64){
        while(commandport.Read() & 0x1){  //清空操作  控制寄存器为1 表示输出buffer满，输入buffer空，即有东西 先读取掉
            dataport.Read();
        }
        commandport.Write(0xae);//激活键盘
        commandport.Write(0x20);//准备读取8042芯片的command byte
        uint8_t status = (dataport.Read() | 1) & ~0x10; //获取状态 开启键盘中断并且开启键盘 --->键盘控制器
        commandport.Write(0x60);//写入状态之前 需要告诉硬件我要写入了
        dataport.Write(status);
        dataport.Write(0xf4);//告诉8048 写完了 清空输出buffer
...
uint32_t KeyBoardDriver::HandleInterrupt(uint32_t esp){
    ...
    //键盘按下和弹起都会激起一个响应 需要过滤掉弹起的响应（因为我们只要一个回应）
    //同时实现一些特殊操作，比如按shift+其他按键的操作--->键盘扫描码表
    switch(key){
...
*/
        ...
        case 0x2A: case 0x36: shift = true; break; //左右shift 按下 设置为true
        case 0xAA: case 0xB6: shift = false; break;//左右shift 弹起 设置为false
